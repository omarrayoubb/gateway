...
export class AccountsService {
   ...
   */
  async create(createAccountDto: CreateAccountDto, currentUser: { id: string; name: string; email: string }): Promise<AccountCreateResponse> {
    ...
    // userIds is now required, so we always fetch the specified users
  ...
  async bulkUpdate(
    ...
    let usersToAssign: User[] | null = null;
    if (updateFields.userIds) {
      usersToAssign = await this.userRepository.findBy({
