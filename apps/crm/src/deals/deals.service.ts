...
export class DealsService {
   ...
   * Creates a new deal
   */
  async create(createDealDto: CreateDealDto, currentUser: Omit<User, 'password'>): Promise<DealResponseDto> {
    ...
    const owner = await this.userRepository.findOneBy({ id: createDealDto.ownerId });
    if (!owner) {
  ...
  async update(
    ...
    if (updateDealDto.ownerId) {
      const owner = await this.userRepository.findOneBy({ id: updateDealDto.ownerId });
      if (!owner) {
  ...
  private _transformDealToResponse(deal: Deal): DealResponseDto {
    ...
    // Build owner name parts
    let ownerFirstName = '';
    let ownerLastName = '';
    if (deal.owner?.name) {
      const nameParts = deal.owner.name.split(' ');
